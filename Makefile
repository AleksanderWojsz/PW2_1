...
MAIN_SRC := main.c # Zmiana
MOJA_H := moja.h # Dodana ścieżka do twojej biblioteki

all: mimpirun $(EXAMPLES) $(TESTS) main # Zmiana

mimpirun: $(MIMPIRUN_SRC)
	gcc $(CFLAGS) -o $@ $(filter %.c,$^)

examples_build/%: examples/%.c $(MIMPI_SRC) $(MOJA_H)
	mkdir -p examples_build
	gcc $(CFLAGS) -o $@ $(filter %.c,$^)

main: $(MAIN_SRC) $(MIMPI_SRC) $(MOJA_H) # Zmiana
	gcc $(CFLAGS) -o $@ $(filter %.c,$^)
...
clean:
	rm -rf mimpirun assignment.zip examples_build main # Zmiana
